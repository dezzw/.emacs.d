...
nix run .#demacs-master        # Git master without patches
nix run .#demacs-master-patched # Git master with patches
#+end_src
...
| =demacs-master=            | Git master build               |
| =demacs-master-patched=    | Git master with macOS patches  |
